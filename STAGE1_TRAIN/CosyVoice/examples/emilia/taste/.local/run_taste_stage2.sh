...
# stage2 must initialize from stage1
stage=5
stop_stage=5
...
ckpt_fpath="/proj/mtklmadm/dev/mtk53678/rtslm_storage/code/rtslm/CosyVoice/examples/emilia/taste/exp/llm/torch_ddp/stage2/0127_stg2_eos-drop_rvq-d256-l4-k512_sum/checkpoint_best.pt"
conf_fpath="/proj/mtklmadm/dev/mtk53678/rtslm_storage/code/rtslm/CosyVoice/examples/emilia/taste/conf/stage2/$EXP_NAME.yaml"
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
  ...
  for model in llm; do
    mkdir -p `pwd`/exp/$model/$train_engine/stage2/$EXP_NAME
    cp $0 `pwd`/exp/$model/$train_engine/stage2/$EXP_NAME
    NCCL_DEBUG=INFO OMP_NUM_THREADS=$_OMP_NUM_THREADS torchrun --nnodes=1 --nproc_per_node=$num_gpus \
        --rdzv_id=$job_id --rdzv_backend="c10d" --rdzv_endpoint="localhost:0" \
      cosyvoice/bin/train.py \
      --train_engine $train_engine \
      --config $conf_fpath \
      --train_data $DATA_ROOT/train_with_libritts.data.list \
      --cv_data $DATA_ROOT/dev.data.list \
      --model $model \
      --model_dir `pwd`/exp/$model/$train_engine/stage2/$EXP_NAME \
      --tensorboard_dir `pwd`/tensorboard/$model/$train_engine/stage2/$EXP_NAME \
